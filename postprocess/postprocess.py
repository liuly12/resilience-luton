...
flows = pd.read_csv(os.path.join(results_folder, 'flows.csv')).groupby('arc')
...
if True:
    ...
    for nrfa_id, obs_flow in val_flows.items():
        figs, ax1 = plt.subplots(1, 1, figsize=(fig_width,fig_height))
        ...
        ax1.legend()
        ax1.set_ylabel('Flow (m3/s)', size = 30)
...
# %% gw head
if True:
    tanks = pd.read_csv(os.path.join(results_folder, 'tanks.csv')).groupby('node')
...
if True:
    ...
    val_wq_lookup_england = {'DIN' : 'Nitrogen, Total Oxidised as N',
                              'SRP' : 'Orthophosphate, reactive as P',
                              'NH3-N' : 'Ammoniacal Nitrogen as N'
                              }
    
    metric_df = pd.DataFrame(index = val_wq_lookup_england.keys(), columns = ['nse', 'rmse', 'r', 'ksg', 'mape', 'pb', 'no_obs'])
    for var in val_wq_lookup_england.keys():
        fig_width = 36
        fig_height = 7.5
        dpi = 300
        fig_num = 1
        figs, ax1 = plt.subplots(fig_num, 1, figsize=(fig_width,fig_height*fig_num))
        ...
        ax1.yaxis.grid(visible=True, which='major', color='k', linestyle='-', alpha=0.2)
        ax1.yaxis.grid(visible=True, which='minor', color='k', linestyle='-', alpha=0.2)
        # ax1.set_title(wq_station, fontsize = 30)
        ...
        if len(ind) > 0:
            ...
            metric_df.loc[node, 'nse'] = (1 - ((obs.sub(sim))**2).mean()/((obs.sub(obs.mean()))**2).mean())[0]
            metric_df.loc[node, 'rmse'] = np.sqrt(metrics.mean_squared_error(obs, sim))
            metric_df.loc[node, 'r'] = r
            metric_df.loc[node, 'mape'] = (np.mean(np.abs((sim - obs) / obs), axis=0) * 100)[0]
            metric_df.loc[node, 'ksg'] = 1 - math.sqrt((r - 1) ** 2 + (sim.std() / obs.std() - 1) ** 2 + (sim.mean() / obs.mean() - 1) ** 2)
            metric_df.loc[node, 'pb'] = (sim.sub(obs).sum() / obs.sum())[0]
            metric_df.loc[node, 'no_obs'] = len(obs)
    
    metrics_['wq'] = metric_df
# %%
# validate cso
if True:
    ...
    for cso_type, info in cso_types.items():
        figs, ax1 = plt.subplots(1, 1, figsize=(fig_width,fig_height))
        ax1.set_title(cso_type, fontsize = 30)
